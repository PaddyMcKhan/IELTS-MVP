...
const supabase = supabaseUrl && supabaseKey ? (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$supabase$2f$supabase$2d$js$2f$dist$2f$module$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["createClient"])(supabaseUrl, supabaseKey) : null;
async function POST(req) {
    try {
        ...
        let userId = null;
        // If logged in → look up matching row in Supabase "User" table
        if (session?.user?.email) {
            const { data: userRow, error: userError } = await supabase.from("User") // ← your existing Supabase table
...
async function GET(req) {
    try {
        ...
        let userId = null;
        if (session?.user?.email) {
            const { data: userRow } = await supabase.from("User").select("id").eq("email", session.user.email).maybeSingle();
            if (userRow?.id) userId = userRow.id;
        }
        if (!userId) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        const { data, error } = await supabase.from("essay_attempts").select("*").eq("user_id", userId).order("created_at", {
            ascending: false
        }).limit(20);
        if (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__88a8ab61._.js.map